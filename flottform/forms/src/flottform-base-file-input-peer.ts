...
import { BaseFileInputPeerEvents, EventEmitter, Logger } from './internal';
...
type ChannelType = FlottformChannelHost | FlottformChannelClient;

export abstract class FlottformBaseFileInputPeer<
	EventMap extends BaseFileInputPeerEvents,
	T extends ChannelType
> extends EventEmitter<EventMap> {
	protected channel: T | null = null;
	protected incomingInputField?: HTMLInputElement; // Contains files received from the other peer.
	protected outgoingInputField?: HTMLInputElement; // Contains files to send to the other peer.
	// Sending files state (Peer1 -> Peer2)
	protected sendState: FileSendState = {
		metaData: [],
		arrayBuffers: [],
		currentFileIndex: 0,
		currentChunkIndex: 0
	};
	// Receiving files state (Peer2 -> Peer1)
	protected receiveState: FileReceiveState = {
		metaData: [],
		currentFile: null,
		receivedSize: 0,
		totalSize: 0
	};
	protected chunkSize: number = 16384; // 16 KB chunks
	...
	protected createMetaData = (inputElement: HTMLInputElement): MetaData | null => {
	...
	protected createArrayBuffers = async (inputElement: HTMLInputElement) => {
		if (!inputElement.files) return null;
	...
	protected startSendingFiles = () => {
		this.sendNextChunk();
	};
	protected sendNextChunk = async () => {
	...
	protected handleIncomingData = (e: MessageEvent) => {
	...
	protected appendFileToInputField = (fileIndex: number) => {
		...
		if (!this.incomingInputField.multiple) {
			this.logger.warn(
				"The current peer's input field only supports one file. Incoming files from the other peer will overwrite any existing file, and only the last file received will remain attached."
			);
			dt.items.clear();
		}
